      ...
      {isImage && (
        <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
          <DialogContent
            ...
            className="p-0 bg-transparent border-none shadow-none max-w-full max-h-full flex items-center justify-center"
          >
            <VisuallyHidden>
              <DialogTitle>Image preview</DialogTitle>
            </VisuallyHidden>
            <DialogClose asChild>
              <button className="absolute top-4 right-4 text-white/80 hover:text-white bg-black/40 hover:bg-black/60 rounded-full p-2 transition cursor-pointer" >
                <X className="w-5 h-5" />
              </button>
            </DialogClose>
            <img
